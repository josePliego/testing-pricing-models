...
from src.tuners import fit_nested_model
import pandas as pd
import warnings
import optuna
...
model_path = SOURCE_DIR + 'output/models/nested_full/'
...
def iterate(ext):
    ...
    fit_nested_model(
        dt_trn, dt_val, model_name, model_path, optuna_seed=iterable.index(ext), nfolds=3
    )
